...
func ParseAccountFromOrganizationsAccountARN(arn string) (string, error) {
	...
	if arnParts.ResourceType != "account" || arnParts.Service != organizationsServiceName {
		return "", trace.BadParameter("invalid organizations account arn (service or resource type): %v", arnParts)
	}
	...
	if len(resourcePaths) != 2 {
		return "", trace.BadParameter("invalid organizations account arn (resource format): %v", arnParts)
	}
	...
	if !strings.HasPrefix(organizationID, "o-") {
		return "", trace.BadParameter("invalid organizations account arn (organization ID format): %s", organizationID)
	}
	...
	if len(organizationIDWithoutPrefix) < 10 || len(organizationIDWithoutPrefix) > 32 {
		return "", trace.BadParameter("invalid organizations account arn (organization ID length): %s", organizationID)
	}
